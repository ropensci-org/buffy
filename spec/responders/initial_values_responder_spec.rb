...
describe InitialValuesResponder do
  ...
  describe "configurations" do
    ...
    it "should allow warning for empty values" do
      ...
      expect(@responder).to_not receive(:update_issue)
      expect(@responder).to receive(:respond).with("Your submission template is missing the following values: version")
      @responder.process_message("")
    end
    ...
    it "should allow warning for non-present values" do
      ...
      expect(@responder).to receive(:respond).with("Your submission template is missing the following values: archive")
      @responder.process_message("")
    end
    ...
    it "should allow multiple warnings" do
      ...
      expect(@responder).to receive(:update_issue)
      expect(@responder).to receive(:respond).with("Your submission template is missing the following values: archive, version")
      @responder.process_message("")
    end
  end
